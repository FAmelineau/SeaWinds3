---
title: "Cluster analysis BLKI spring"
author: "FA"
date: "24 2 2022"
...
ggplot(bird_data_year) + geom_histogram(aes(x=  mon_wspeed)) + 
  geom_vline(xintercept = mean(bird_data_year$mon_wspeed), col = "red") +
  geom_vline(xintercept = median(bird_data_year$mon_wspeed)) +
  labs(title = "Wind speed, monthly mean (scalar) (m/s)")
...
get_clust_tendency(nor, 14, graph = FALSE)
...
# clustnames = c("SW","Barents","Skl_Røs_And","Bjø_KG")

# clustnames = c( "IoM_Run_Skli","Bjø_KG","Ice_Far_Røs","Spitz_FJL","And_Hor_CK")
clustnames = c( "A","C","B","D") 
...
bird_data_year_spring = bird_data_year %>% filter(season=="spring" & n_ringyear>=5)


bird_data_col$clustname = factor(clustnames[bird_data_col$cluster], 
                                  levels = c("A","B","C","D")  )

bird_data_col_spring = bird_data_col %>% filter(season=="spring" & n_ringyear>=5)
...
top_row = plot_grid(map, plot1, labels = c("c","d") , rel_widths = c(1.7, 1))

print(top_row) 

ggsave("Fig_clust_1_BLKI_spring_samescale.jpg", plot = top_row , width = 6, height = 3, units = "in")
...
ggsave("Fig_clust_BLKI_spring_supp_v3.png", plot = final , width = 8, height = 9, units = "in")
...
write.csv(sum, file = "summary_BLKI_spring.csv")
